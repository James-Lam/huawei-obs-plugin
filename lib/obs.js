...
class OBS {
  constructor(options) {
    ...
    if (
      ["accessKeyId", "secretAccessKey", "bucket", "server"].some(
        (key) => !options[key]
      )
    ) {
      throw new Error(
        `请填写正确的accessKeyId、secretAccessKey、bucket、server`
      );
    }
        ...
        if (filters.length) {
          ...
          if (delRes.CommonMsg.Status < 300 && delRes.InterfaceResult) {
            ...
            for (let i = 0; i < delRes.InterfaceResult.Deleteds.length; i++) {
              log(
                colors.blue(
                  `删除 ${delRes.InterfaceResult.Deleteds[i]["Key"]} 成功!`
                )
              );
            }
            // 获取删除失败的对象
            for (let i = 0; i < delRes.InterfaceResult.Errors.length; i++) {
              log(
                colors.red(
                  `删除 ${delRes.InterfaceResult.Errors[i]["Key"]} 失败!`
                )
              );
            }
          } else {
